...
const ProductCatalog = {
  ...
  // Unified reactive filtering pipeline for products.html
  applyFilters() {
    ...
    if (this.state.category && this.state.category !== 'all') {
      list = list.filter(item => item.category === this.state.category);
    }

    // 2. Search Query (matches name, origin, tasting notes, description, badge, category, season)
    ...
    // 4. Certifications
    if (this.state.certOrganic) {
      list = list.filter(item => 
        item.category === 'organic' ||
        (item.badge && item.badge.toLowerCase().includes('organic')) ||
        (item.description && item.description.toLowerCase().includes('organic'))
      );
    }

    if (this.state.certComb) {
      list = list.filter(item => 
        item.category === 'comb' ||
        (item.badge && item.badge.toLowerCase().includes('comb')) ||
        (item.name && item.name.toLowerCase().includes('comb'))
      );
    }

    if (this.state.certVegan) {
      list = list.filter(item => 
        item.category === 'sweeteners' ||
        (item.badge && item.badge.toLowerCase().includes('vegan')) ||
        (item.description && item.description.toLowerCase().includes('vegan'))
      );
    }
    ...
    // 6. Render
    if (list.length === 0) {
      const queryDisplay = this.state.searchQuery ? ` for "<strong>${this.escapeHtml(this.state.searchQuery)}</strong>"` : '';
      container.innerHTML = `
        <div class="col-12 text-center py-5">
          <div class="mb-3">
            <i class="fas fa-search fa-3x text-warning opacity-75"></i>
          </div>
          <h4 class="font-serif mb-2">No Matching Varieties Found</h4>
          <p class="text-muted mb-4">We couldn't find any honeys or sweeteners matching your criteria${queryDisplay}.</p>
          <button class="btn btn-honey" onclick="ProductCatalog.resetFilters()">
    ...
    container.innerHTML = list.map(item => this.renderCard(item)).join('');
    if (window.WishlistController) window.WishlistController.updateUI();
  },
  ...
  resetFilters() {
    ...
    this.state.maxPrice = 40;
    this.state.certOrganic = false;
    this.state.certComb = false;
    this.state.certVegan = false;
    ...
    if (priceSlider) priceSlider.value = 40;
    if (priceDisplay) priceDisplay.textContent = '$40';

    const orgCheck = document.getElementById('cert-organic');
    const combCheck = document.getElementById('cert-comb');
    const vegCheck = document.getElementById('cert-vegan');
    if (orgCheck) orgCheck.checked = false;
    if (combCheck) combCheck.checked = false;
    ...
    document.querySelectorAll('[data-product-filter]').forEach(b => {
      if (b.getAttribute('data-product-filter') === 'all') {
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.querySelectorAll('[data-product-filter]').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      ProductCatalog.state.category = btn.getAttribute('data-product-filter');
      ProductCatalog.applyFilters();
    });
  });

  // Search input filter with instant real-time response
  const searchInput = document.getElementById('product-search-input');
  if (searchInput) {
    searchInput.addEventListener('input', (e) => {
      ProductCatalog.state.searchQuery = e.target.value;
      ProductCatalog.applyFilters();
    });
  }
  ...
  if (priceSlider && priceDisplay) {
    priceSlider.addEventListener('input', (e) => {
      ...
      ProductCatalog.state.maxPrice = maxPrice;
  ...
  const certOrganic = document.getElementById('cert-organic');
  if (certOrganic) {
    certOrganic.addEventListener('change', (e) => {
      ProductCatalog.state.certOrganic = e.target.checked;
      ProductCatalog.applyFilters();
    });
  }

  const certComb = document.getElementById('cert-comb');
  if (certComb) {
    certComb.addEventListener('change', (e) => {
      ProductCatalog.state.certComb = e.target.checked;
      ProductCatalog.applyFilters();
    });
  }

  const certVegan = document.getElementById('cert-vegan');
  if (certVegan) {
    certVegan.addEventListener('change', (e) => {
      ProductCatalog.state.certVegan = e.target.checked;
      ProductCatalog.applyFilters();
    });
  }
  ...
  if (sortSelect) {
    sortSelect.addEventListener('change', (e) => {
      ProductCatalog.state.sortBy = e.target.value;
      ProductCatalog.applyFilters();
    });
  }
});
